...
const TempDetail = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const res = await axios.get(
          `/api/environment/temp?interval=${interval}`
        );
        const result = res.data.map((item) => ({
          time: new Date(item.joinDate).toLocaleTimeString(),
          온도: item.temperature,
        }));
        setData(result); /* time과 온도를 받아오는 것*/

        // ✅ 적정 온도 판별
        console.log();
        if (crop.tempMin !== undefined && crop.tempMax !== undefined) {
          const allInRange = result.every(
            (d) => d.온도 >= crop.tempMin && d.온도 <= crop.tempMax
          );
    ...
    fetchHistory();
  }, [interval, crop.tempMin, crop.tempMax]);
        ...
        {isTemperatureOk ? (
          <>
            ...
            <div className={styles.statusGood}>
              <span style={{ color: "green" }}>✅ 온도가</span>
          ...
          <>
            ...
            <div className={styles.statusGood}>
              <span style={{ color: "red" }}>⚠️ 온도가</span>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data}>
            ...
            <ReferenceArea
              ...
              fillOpacity={0.2}
              label={{
                value: `${crop.crop} 적정 온도 (${crop.tempMin} ~ ${crop.tempMax}°C) `,
